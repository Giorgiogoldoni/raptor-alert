...
"""
RAPTOR Alert v3.0
- Logica BUY1/BUY2/BUY3 basata su derivata KAMA + SAR (come Chart)
- BOND categoria separata con logica WATCH/ATTENZIONE/STOP
- Salva data/TICKER.json per ogni ETF (barre 2y + indicatori)
- Salva data/etf.json index leggero
- Simulazione trade 2 anni + perf stats
- Mail novità via Gmail SMTP
...
# Categorie BOND — logica separata
BOND_CATS = {'BOND', 'Liquidità'}
...
def kama_derivative(kama_arr, i, min_pct=0.0001):
    """Conta barre consecutive in salita. Ritorna: >0 = salita, 0 = piatta, -1 = scende"""
    ...
    for j in range(i, max(0, i-6), -1):
        ...
        if diff > 0 and pct > min_pct: count += 1
        else: break
    if count == 0:
...
def calc_sar(high, low, af_start=0.02, af_max=0.20):
    n = len(high)
    sar = [0.0]*n; trend = [1]*n
    ep = low[0]; af = af_start; sar[0] = high[0]
    for i in range(1, n):
        ps, pt = sar[i-1], trend[i-1]
        if pt == 1:
            sar[i] = ps + af*(ep-ps)
            ...
            if low[i] < sar[i]:
                ...
                trend[i]=1
                if high[i] > ep: ep=high[i]; af=min(af+af_start, af_max)
        else:
            sar[i] = ps + af*(ep-ps)
            ...
            if high[i] > sar[i]:
                ...
                trend[i]=-1
                if low[i] < ep: ep=low[i]; af=min(af+af_start, af_max)
    return sar, trend

def sar_streak_info(trend, t):
    n = len(trend)
    cur = trend[-1]
    i = n-1
    while i>0 and trend[i-1]==cur:
        i -= 1
    count = n - i
    since_ts = t[i] if i < len(t) else None
    since = datetime.fromtimestamp(since_ts, tz=timezone.utc).strftime('%d/%m/%Y') if since_ts else None
    return count, since

def calc_ao(high, low):
    mid = [(h+l)/2 for h,l in zip(high,low)]
    e7 = ema_arr(mid,7); e34 = ema_arr(mid,34)
...
def get_sar_step(close, atr_vals, lookback=60):
    last = len(close)-1
    start = max(0, last-lookback)
    vals = [atr_vals[i]/close[i]*100 for i in range(start,last+1)
            if atr_vals[i] is not None and close[i]>0]
    if not vals: return 0.02, 0.0
    avg = sum(vals)/len(vals)
    if avg < 1.0: step = 0.020
    elif avg < 2.0: step = 0.015
    elif avg < 3.5: step = 0.012
    else: step = 0.010
    return step, round(avg,3)
...
# ── Logica segnale AZIONARIO (BUY1/2/3) ──────────────────────────────────────
def calc_signal_equity(i, close, kama_v, kama_s, sar_arr, sar_trend, ao, er_arr, trd):
    """
    BUY1: SAR bull + KAMA veloce sale 2+ barre + ER>0.25 + prezzo>KAMA lenta
    BUY2: BUY1 + AO>0 + ER>0.35 + baff>=2
    BUY3: BUY2 + KAMA veloce sale 3+ barre + ER>0.45 + baff>=3 + trendycator VERDE
    EXIT1: SAR bear + prezzo > KAMA lenta (attenzione)
    EXIT2: prezzo < KAMA lenta (uscita)
    STOP:  prezzo < KAMA lenta + SAR bear
    """
    if i < 40: return 'NONE', ''
    price  = close[i]
    kv     = kama_v[i]
    ks     = kama_s[i]
    if kv is None or ks is None: return 'NONE', ''

    sar_up    = sar_trend[i] == 1
    above_kv  = price > kv
    above_ks  = price > ks
    kf_deriv  = kama_derivative(kama_v, i)
    kfr2      = kf_deriv >= 2
    kfr3      = kf_deriv >= 3
    er        = er_arr[i] if i < len(er_arr) else 0

    # AO e baffetti
    ao_pos    = ao[i] > 0 if ao[i] is not None else False
    baff = 0
    for j in range(i, max(0,i-10), -1):
        if ao[j] is not None and ao[j-1] is not None and ao[j]>ao[j-1]: baff+=1
        else: break

    # EXIT
    if not above_ks and not sar_up: return 'NONE', 'STOP'
    if not above_ks: return 'NONE', 'USCITA'
    if not sar_up and above_ks: return 'NONE', 'ATTENZIONE'

    # BUY — richiede prezzo > KAMA lenta
    if not above_ks: return 'WATCH', ''

    if sar_up and kfr3 and ao_pos and er>0.45 and baff>=3 and trd=='VERDE':
        return 'BUY3', ''
    if sar_up and kfr2 and ao_pos and er>0.35 and baff>=2:
        return 'BUY2', ''
    if sar_up and kfr2 and er>0.25:
        return 'BUY1', ''
    return 'WATCH', ''

# ── Logica segnale BOND ───────────────────────────────────────────────────────
def calc_signal_bond(price, kama_s, sar_now, sar_up):
    """
    BOND: logica semplificata solo SAR + KAMA lenta
    WATCH:      SAR bull + prezzo > KAMA lenta
    ATTENZIONE: prezzo < KAMA lenta
    STOP:       SAR bear + prezzo < KAMA lenta
    """
    if kama_s is None: return '', ''
    above_ks = price > kama_s
    if not above_ks and not sar_up: return '', 'STOP'
    if not above_ks: return '', 'USCITA'
    if above_ks and not sar_up: return 'WATCH', 'ATTENZIONE'
    if above_ks and sar_up: return 'WATCH', ''
    return '', ''

# ── Simulazione trade ─────────────────────────────────────────────────────────
GRACE = 3

def simulate_trades(bars, signals, uscite):
    trades = []
    in_trade = False; ent_i = 0; ent_p = 0; ent_sig = ''; days = 0
    for i,(bar,sig,usc) in enumerate(zip(bars,signals,uscite)):
        price = bar['close']
        if not in_trade:
            if sig in ('BUY1','BUY2','BUY3','WATCH'):
                in_trade=True; ent_i=i; ent_p=price; ent_sig=sig; days=0
        else:
            days+=1
            eff_usc = usc if days>GRACE else ''
            is_exit = eff_usc in ('STOP','USCITA') or i==len(bars)-1
            if is_exit:
                xs = 'OPEN' if (i==len(bars)-1 and eff_usc not in ('STOP','USCITA')) else eff_usc
                pnl = (price-ent_p)/ent_p*100
                trades.append({
                    'entSig':ent_sig,'exitSig':xs,
                    'entTime':bars[ent_i]['time'],'exitTime':bar['time'],
                    'entPrice':round(ent_p,4),'exitPrice':round(price,4),
                    'pnlPct':round(pnl,4),'isOpen':xs=='OPEN','days':days
                })
                in_trade=False
    return trades

def perf_stats(trades):
    cl = [t for t in trades if not t['isOpen']]
    if not cl: return {'trades':len(trades),'closed':0,'wins':0,'wr':0,'totalPnl':0,'best':0,'worst':0,'avg':0,'dd':0}
    wins = [t for t in cl if t['pnlPct']>0]
    pnls = [t['pnlPct'] for t in cl]
    tot  = sum(pnls)
    peak=eq=dd=0
    for p in pnls:
        eq+=p
    ...
    return {
        'trades':len(trades),'closed':len(cl),'wins':len(wins),
        'wr':round(len(wins)/len(cl)*100,1) if cl else 0,
        'totalPnl':round(tot,2),
        'best':round(max(pnls),2),'worst':round(min(pnls),2),
        'avg':round(tot/len(cl),2) if cl else 0,'dd':round(dd,2)
    }

# ── Analisi singolo ticker ────────────────────────────────────────────────────
def analyze(info):
    base = {"ticker":info["t"],"display":info["t"],"categoria":info["c"],
            "error":None,"score":0,"tipo":"","uscita":"","is_bond":False}
    raw = fetch_yahoo(info["y"])
    if not raw: return {**base,"error":"fetch failed"}

    try:
        q = raw["indicators"]["quote"][0]
        ts_raw = raw["timestamp"]
        cr=q["close"]; hr=q["high"]; lr=q["low"]; vr=q["volume"]
        c,h,l,v,t=[],[],[],[],[]
        for i in range(len(ts_raw)):
            if cr[i] and hr[i] and lr[i]:
                c.append(cr[i]);h.append(hr[i]);l.append(lr[i])
                v.append(vr[i] or 0);t.append(ts_raw[i])
        if len(c)<60: return {**base,"error":"Dati insuff."}

        is_bond = info["c"] in BOND_CATS

        # ── Indicatori ──
        kama_v = calc_kama(c, n=5,  fast=2, slow=10)
        kama_s = calc_kama(c, n=21, fast=2, slow=30)
        atr_v  = calc_atr(h,l,c,14)
        sar_step, atr_pct = get_sar_step(c, atr_v, 60)
        sar_arr, sar_trend = calc_sar(h, l, af_start=sar_step)
        sar_count, sar_since = sar_streak_info(sar_trend, t)
        ao     = calc_ao(h,l)
        er     = calc_er(c)
        trd    = trendycator(c)
        rsi    = calc_rsi(c)
        mm20   = sma(c,20); mm50=sma(c,50); mm200=sma(c,200)
        ...
        perf_g   = round((price/c[-2]-1)*100,2)  if len(c)>=2  else 0
        perf_s   = round((price/c[-6]-1)*100,2)  if len(c)>=6  else 0
        perf_m   = round((price/c[-23]-1)*100,2) if len(c)>=23 else 0
        pk_pct   = round((price-kv_now)/kv_now*100,2) if kv_now else 0
        vr2      = vol_ratio(v)
        kf_deriv = kama_derivative(kama_v, len(c)-1)

        # ATR ultimo
        atr_last     = atr_v[-1]
        atr_pct_last = round(atr_last/price*100,3) if atr_last and price else None
        if atr_pct < 1.0: vol_class='BASSA'
        elif atr_pct < 2.0: vol_class='MEDIA'
        elif atr_pct < 3.5: vol_class='ALTA'
        else: vol_class='ESTREMA'

        # ── Segnale ──
        er_arr = [calc_er(c[:i+1]) if i>=10 else 0 for i in range(len(c))]

        if is_bond:
            tipo, uscita = calc_signal_bond(price, ks_now, sar_now, sar_up)
        else:
            tipo, uscita = calc_signal_equity(
                len(c)-1, c, kama_v, kama_s, sar_arr, sar_trend, ao, er_arr, trd)
        ...
        if is_bond:
            reasons = [
                {"label":"SAR rialzista",      "ok":sar_up},
                {"label":"Prezzo > KAMA lenta","ok":above_ks},
                {"label":"AO positivo",        "ok":ao_now>0},
            ]
        else:
            reasons = [
                {"label":"SAR rialzista",           "ok":sar_up},
                {"label":"KAMA veloce sale 2+ barre","ok":kf_deriv>=2},
                {"label":"KAMA veloce sale 3+ barre","ok":kf_deriv>=3},
                {"label":"Prezzo > KAMA lenta",     "ok":above_ks},
                {"label":"ER > 0.25",               "ok":er>0.25},
                {"label":"ER > 0.35",               "ok":er>0.35},
                {"label":"ER > 0.45",               "ok":er>0.45},
                {"label":"Baffetti AO >= 2",        "ok":baff>=2},
                {"label":"Baffetti AO >= 3",        "ok":baff>=3},
                {"label":"AO positivo",             "ok":ao_now>0},
                {"label":"Trendycator VERDE",       "ok":trd=='VERDE'},
                {"label":"MM allineate",            "ok":mm_align},
                {"label":"RSI > 50",                "ok":rsi is not None and rsi>50},
            ]

        # ── Score composito ──
        if is_bond:
            score = int(max(0,min(100,round(
                (10 if sar_up else 0)+(10 if above_ks else 0)+(5 if ao_now>0 else 0)
            ))))
        else:
            score = int(max(0,min(200,round(
                er*30+min(baff,10)*5+min(abs(pk_pct),5)*3
                +max(-10,min(5,perf_s))*4+max(-20,min(10,perf_m))*2
                +(10 if mm_align else 0)+(5 if ao_now>0 else 0)+(5 if sar_up else 0)
                +(20 if cross<=3 else 12 if cross<=10 else 5 if cross<=20 else 0)
            ))))

        # ── Barre per grafico (ultime 500 o tutte) ──
        bars_out = []
        for i in range(len(c)):
            bars_out.append({
                'time': t[i],
                'open':  round(c[i],4),   # semplificato: open=close per ETF
                'high':  round(h[i],4),
                'low':   round(l[i],4),
                'close': round(c[i],4),
                'volume':int(v[i])
        ...
        # Indicatori come array per grafico
        kama_v_out = [round(x,4) if x is not None else None for x in kama_v]
        kama_s_out = [round(x,4) if x is not None else None for x in kama_s]
        sar_out    = [round(x,4) for x in sar_arr]
        ...
        for i in range(len(c)):
            if is_bond:
                sg, us = calc_signal_bond(c[i], kama_s[i], sar_arr[i], sar_trend[i]==1)
            else:
                sg, us = calc_signal_equity(i,c,kama_v,kama_s,sar_arr,sar_trend,ao,er_arr,trd)
            signals_all.append(sg)
            uscite_all.append(us)

        trades = simulate_trades(bars_out, signals_all, uscite_all)
        perf   = perf_stats(trades)

        # Momentum days
        momentum_days = 0
        for sg in reversed(signals_all):
            if sg in ('BUY1','BUY2','BUY3','WATCH'): momentum_days+=1
            else: break

        composite_score = round(er * score * math.log(momentum_days+2), 4)

        return {
            "ticker":info["t"],"display":info["t"],"categoria":info["c"],
            "is_bond":is_bond,
            "price":entry_price,"kama_v":round(kv_now,4) if kv_now else None,
            "kama_s":round(ks_now,4) if ks_now else None,
            "sar":round(sar_now,4),"sar_up":sar_up,"sar_step":sar_step,
            "sar_count":sar_count,"sar_since":sar_since,
            "er":round(er,3),"baff":baff,"kf_deriv":kf_deriv,
            "trendycator":trd,"rsi":rsi,
            "mm20":round(mm20,4) if mm20 else None,
            "mm50":round(mm50,4) if mm50 else None,
            "mm200":round(mm200,4) if mm200 else None,
            "mm_align":mm_align,"ao_pos":ao_now>0,
            "pk_pct":pk_pct,"perf_giorno":perf_g,"perf_sett":perf_s,"perf_mese":perf_m,
            "vol_ratio":vr2,"score":score,"composite_score":composite_score,
            "tipo":tipo,"uscita":uscita,"cross":cross,
            "momentum_days":momentum_days,
            "atr_pct":atr_pct_last,"vol_class":vol_class,
            "entry_price":entry_price,"entry_date":NOW_DATE,"entry_time":NOW_TIME,
            "stop_price":stop_price,"target_price":target_price,
            "risk_pct":risk_pct,"reward_pct":reward_pct,
            "reasons":reasons,"perf":perf,"trades":trades[-20:],
            "error":None,
            # Array per grafico (salvati nel file individuale)
            "_bars":bars_out,"_kama_v":kama_v_out,"_kama_s":kama_s_out,
            "_sar":sar_out,"_sar_trend":sar_trend,"_ao":ao,
            "_signals":signals_all,"_uscite":uscite_all,
        }
    except Exception as e:
        return {**base,"error":str(e)}

# ── Storico ───────────────────────────────────────────────────────────────────
TIPO_MAP = {"LONG":"BUY3","EARLY":"BUY2","WATCH":"BUY1","ROSSO+":"WARNING"}

def load_history():
    try:
        with open("data/history.json","r") as f: return json.load(f)
    except: return {}

def save_history(h):
    os.makedirs("data",exist_ok=True)
    with open("data/history.json","w") as f: json.dump(h,f,ensure_ascii=False,indent=2)

def update_history(results, history):
    news={"new_buy3":[],"new_buy2":[],"new_buy1":[],"new_warning":[],
          "new_stop":[],"new_exit":[],"new_att":[]}
    for r in results:
        if r.get("error") and not r.get("price"): continue
        tk=r["ticker"]; tipo=r.get("tipo",""); usc=r.get("uscita","")
        prev=history.get(tk,{})
        # Migrazione vecchi nomi
        prev_tipo = TIPO_MAP.get(prev.get("tipo",""), prev.get("tipo",""))
        prev_usc  = prev.get("uscita","")

        if tipo!=prev_tipo:
            entry={"ticker":tk,"categoria":r["categoria"],"tipo":tipo,
                   "entry_price":r.get("entry_price"),"entry_date":NOW_DATE,
                   "entry_time":NOW_TIME,"stop_price":r.get("stop_price"),
                   "target_price":r.get("target_price"),"score":r.get("score")}
            if tipo=="BUY3":   news["new_buy3"].append(entry)
            if tipo=="BUY2":   news["new_buy2"].append(entry)
            if tipo=="BUY1":   news["new_buy1"].append(entry)
            if tipo=="WARNING":news["new_warning"].append(entry)

        if usc!=prev_usc:
            ex={"ticker":tk,"categoria":r["categoria"],"uscita":usc,
                "exit_price":r.get("price"),"exit_date":NOW_DATE,"exit_time":NOW_TIME,
                "entry_price":prev.get("entry_price"),"entry_date":prev.get("entry_date","—"),
                "result_pct":round((r["price"]-prev["entry_price"])/prev["entry_price"]*100,2)
                             if prev.get("entry_price") else None}
            if usc=="STOP":       news["new_stop"].append(ex)
            if usc=="USCITA":     news["new_exit"].append(ex)
            if usc=="ATTENZIONE": news["new_att"].append(ex)

        history[tk]={
            "tipo":tipo,"uscita":usc,
...
# ── Mail ──────────────────────────────────────────────────────────────────────
def fmt_entry(e):
    r  = f"  • {e['ticker']} ({e['categoria']})\n"
    r += f"    Entrata: {e['entry_date']} {e['entry_time']}  prezzo {e.get('entry_price','—')}\n"
    r += f"    Stop: {e.get('stop_price','—')}  Target: {e.get('target_price','—')}\n"
    return r

def fmt_exit(e):
    r  = f"  • {e['ticker']} ({e['categoria']})\n"
    r += f"    Uscita: {e['exit_date']} {e['exit_time']}  prezzo {e.get('exit_price','—')}\n"
    if e.get("result_pct") is not None:
        s = "+" if e["result_pct"]>=0 else ""
        r += f"    Risultato: {s}{e['result_pct']}%\n"
    return r

def send_mail(news):
    has_news = any(news[k] for k in news)
    if not has_news: print("  ✉ Nessuna novità"); return
    if not GMAIL_USER or not GMAIL_PASSWORD or not MAIL_TO:
        print("  ✉ Credenziali non configurate"); return

    body = f"🦅 RAPTOR Alert — {NOW}\n{'='*50}\n\n"
    for key,label in [("new_buy3","💎 NUOVI BUY3"),("new_buy2","🔵 NUOVI BUY2"),
                      ("new_buy1","🟢 NUOVI BUY1"),("new_warning","⚠️ NUOVI WARNING"),
                      ("new_stop","⛔ STOP"),("new_exit","🔴 USCITE"),("new_att","🟡 ATTENZIONI")]:
        if news[key]:
            body += f"{label} ({len(news[key])})\n"
            fn = fmt_entry if key.startswith("new_buy") or key=="new_warning" else fmt_exit
            for e in news[key]: body += fn(e)
            body += "\n"
    body += f"\nhttps://giorgiogoldoni.github.io/raptor-alert/\n"

    msg=MIMEMultipart(); msg["From"]=GMAIL_USER; msg["To"]=MAIL_TO
    msg["Subject"]=f"🦅 RAPTOR Alert {NOW} — {sum(len(news[k]) for k in news)} novità"
    msg.attach(MIMEText(body,"plain","utf-8"))
    try:
        with smtplib.SMTP_SSL("smtp.gmail.com",465) as s:
            s.login(GMAIL_USER,GMAIL_PASSWORD); s.send_message(msg)
...
def update_stats(ok, errors):
    os.makedirs("data",exist_ok=True)
    try:
        with open("data/stats.json","r") as f: stats=json.load(f)
    except:
        stats={"run_count_mese":0,"run_count_totale":0,
               "minuti_stimati_mese":0,"reset_mese":NOW_DATE[:5]}
    if stats.get("reset_mese","")!=NOW_DATE[3:5]:
        stats["run_count_mese"]=0; stats["minuti_stimati_mese"]=0
        stats["reset_mese"]=NOW_DATE[3:5]
    stats["run_count_mese"]+=1; stats["run_count_totale"]+=1
    stats["minuti_stimati_mese"]+=5  # 2y range dura di più
    stats["ultimo_run"]=NOW; stats["ultimo_ok"]=ok; stats["ultimo_errori"]=errors
    with open("data/stats.json","w") as f: json.dump(stats,f,ensure_ascii=False,indent=2)
    print(f"  📊 Run mese: {stats['run_count_mese']} · ~{stats['minuti_stimati_mese']} min")
    return stats

# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    print(f"🦅 RAPTOR Alert v3.0 — {NOW}")
    ...
    for i,ticker in enumerate(TICKERS):
        ...
        results.append(r)
        if r["error"]:
            print(f"✗ {r['error']}"); err+=1
        else:
            print(f"✓ {r.get('tipo','—'):5} {r.get('uscita',''):10} "
                  f"score={r.get('score',0):3}  "
                  f"kf_deriv={r.get('kf_deriv',0)}  "
                  f"stop={r.get('stop_price','?')}")
            ok+=1

            # Salva file individuale per grafico (come Chart)
            ticker_file = os.path.join("data", f"{ticker['t']}.json")
            ticker_data = {
                "ticker":  r["ticker"],
                "name":    r["display"],
    ...
    # Rimuovi chiavi private (_bars ecc.) dall'index
    for r in results:
    ...
    print(f"   Novità: {sum(len(news[k]) for k in news)}")

    # Mail
    print("\n✉ Invio mail..."); send_mail(news)

    # etf.json index leggero
    output={
        "updated":NOW,"updated_date":NOW_DATE,"updated_time":NOW_TIME,
        "total":len(results),"ok":ok,"errors":err,"data":results
    }
    ...
    print(f"\n✅ {ok} ok · {err} errori")
    print(f"   data/etf.json ({len(results)} ETF)")
    print(f"   data/TICKER.json ({ok} file individuali)")
    print(f"   Run mese: {stats['run_count_mese']} · ~{stats['minuti_stimati_mese']} min")
